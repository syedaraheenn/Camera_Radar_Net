...
if __name__ == '__main__':
    infos_path = '/home/ivision/CRN/data/nuScenes/nuscenes_infos_test.pkl'
    infos = mmcv.load(infos_path)

    # Specify the scenes to visualize
    target_scenes = {"scene-0936"}

    # Filter only samples belonging to the target scenes
    scene_samples = [info for info in infos if info['scene_name'] in target_scenes]

    print(f"Total samples in {target_scenes}: {len(scene_samples)}")

    # Process only the selected scene samples
    for i, info in enumerate(scene_samples):
        demo_4_tym(
            info,
            i,
            #'/home/ivision/CRN/outputs/det/BEVDepth_r50_256x704_128x128_4key/results_nusc.json',
            '/home/ivision/CRN/outputs/CRN_R50_test/results_nusc.json',
            '/home/ivision/CRN/Test_visualize_0936/',
        )
