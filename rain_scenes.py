...
def get_cam_corners(corners, translation, rotation, cam_intrinsics):
    ...
    # todo: .pkl 파일 찾을 것
    infos = mmcv.load('/home/ivision/CRN/data/nuScenes/nuscenes_infos_val.pkl')
    ...
    for i, k in enumerate(IMG_KEYS):
        ...
        img = mmcv.imread(
            os.path.join('/home/ivision/CRN/data/nuScenes', info['cam_infos'][k]['filename']))
...
def demo_4_tym(
    ...
    results = mmcv.load(nusc_results_file)['results']
    lidar_path = info['lidar_infos']['LIDAR_TOP']['filename']
    lidar_points = np.fromfile(os.path.join('/home/ivision/CRN/data/nuScenes', lidar_path),
                               dtype=np.float32,
                               count=-1).reshape(-1, 5)[..., :4]
    ...
    # Get prediction corners, edited 24.10.19
...
if __name__ == '__main__':
    infos_path = '/home/ivision/CRN/data/nuScenes/nuscenes_infos_train.pkl'  # ✅ Updated file
    scene_json_path = '/home/ivision/CRN/data/nuScenes/v1.0-trainval/scene.json'

    # Load scene.json to get actual rain scenes
    with open(scene_json_path, "r") as f:
        scene_data = json.load(f)

    # Get scene names for rain (scene-440 to scene-480)
    rain_scenes = set(scene['name'] for scene in scene_data if 440 <= int(scene['name'].split('-')[-1]) <= 480)

    print(f"Total rain scenes found in scene.json: {len(rain_scenes)}")

    # Load the dataset info file (train split)
    infos = mmcv.load(infos_path)

    # Filter only the samples belonging to the rain scenes
    rain_samples = [info for info in infos if info['scene_name'] in rain_scenes]

    print(f"Total samples in rain scenes (440-480): {len(rain_samples)}")

    # Process only the rain samples
    for i, info in enumerate(rain_samples):
        demo_4_tym(
            info,
            i,
            '/home/ivision/CRN/outputs/det/CRN_r50_256x704_128x128_4key/results_nusc.json',
            '/home/ivision/CRN/r50_bbox_rain/',
        )
        print(f'Scene {info["scene_name"]} - Sample {i} completed')
